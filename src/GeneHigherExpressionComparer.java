 ...
 */
public class GeneHigherExpressionComparer 
{
    ...
    private static ArrayList<Integer> indicesOfHigherExpression = new ArrayList<Integer>();
    public static void main( String[] args )
    {
        ...
        {
            System.out.println( "Correct Usage: java GeneHigherExpressionComparer SampleToBeCompared's_FileName " + 
        ...
        writeHigherExpressionContigFile(contigArray);
        
        System.out.println("There was found to be " + indicesOfHigherExpression.size() + " contigs where sample one had higher expression than the other two samples");
    }
    ...
    public static void compareSampleValues( double[] sampleArrayToBeCompared, double[] sampleArray2, double[] sampleArray3)
    {
        for( int i = 0; i < sampleArrayToBeCompared.length; i++)
        {
            ...
            {
                indicesOfHigherExpression.add(i);
            }
        }
    }

    public static void writeHigherExpressionContigFile(String[] contigArray)
    {
        try
        {
            BufferedWriter outputFile = 
                new BufferedWriter( new FileWriter( "HigherExpressionContigList") ); 
            for( int i = 0; i < indicesOfHigherExpression.size(); i++)
            {
                outputFile.write( contigArray[indicesOfHigherExpression.get(i)] ); //puts in the contig name for each index where 
